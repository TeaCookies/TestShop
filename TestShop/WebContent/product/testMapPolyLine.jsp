...
// 	    theMarker = L.marker([lat,lon]).addTo(mymap).bindPopup("<b>폴리라인테스트</b>").openPopup();	
		console.log("11111"+latlngs);
	    polyline = L.polyline(latlngs, {color: 'red'}).addTo(mymap);
		console.log("22222"+polyline);
	    if(latlngs.length >=1){
			for (i=0;i<latlngs.length;i++){
		
				theMarker =L.marker(latlngs[i]).addTo(mymap).bindPopup("<b>경로</b>");
		}}
